...
def compute_class_balanced_loss(pred, target, class_names, class_weights, loss_type='smooth_l1'):
    """
    ...
    losses = loss_fn(pred, target)  # [B, 4]
        ...
        if args.mixed_precision:
            with autocast():
                ...
                if args.use_class_weights and class_weights is not None:
                    loss = compute_class_balanced_loss(pred, targets, class_names, class_weights, loss_type='smooth_l1')
                else:
                    loss = model.compute_loss(pred, targets, loss_type='smooth_l1')
